...
<SpecTable
  ...
  rows={[
    ...
    ["alignItemWithTrigger", "boolean", "true", "Overlaps the popup onto the trigger so the selected item's text lands on the trigger's value — native-select behaviour. Set false inside a CSS-transformed ancestor, where that overlap is measured wrong and the popup lands away from its trigger."],
